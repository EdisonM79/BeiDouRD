...
public class ArrayUtils {
  ...
  /**
   * Escapes the bytes in a byte array using JT/TSerial rules.
   * <p>
   * Escapes any value it finds into their JT/TSerial form.
   * <p>
  ...
  /**
   * Unescape any JT/TSerial pattern found in the byte array.
   * <p>
  ...
  @SuppressWarnings("unchecked")
  public static byte[] ensureLowLength(byte[] arr, int len) {
    if (isEmpty(arr)) {
      return new byte[len];
    }
    if (arr.length < len) {
    ...
    if (arr.length > len) {
//      return Arrays.copyOf(arr, len);{
        return Arrays.copyOfRange(arr, arr.length-len, arr.length);
    }
